...
> **Last verified against docs.bluebox.ai:** July 15, 2026
> **Staleness policy, read this before answering any Bluebox question:** This is the newest and fastest-moving product in this entire project, faster than Bindplane. Bluebox was unveiled publicly at AWS Summit New York only weeks before this file's verification date. Its own documentation pages carry version numbers that increment several times a month (e.g., Getting Started was at version 5.8 as of July 13, 2026; Core Concepts at 3.19 as of July 14). Treat every specific detail in this file (CLI flags, supported coding agents, setup steps, permission scopes) as likely to have shifted since verification. Search docs.bluebox.ai fresh before giving a customer-facing or pre-sales answer on Bluebox specifics. Do not rely on this file's content for anything version-sensitive without checking first.
...
Five concepts cover most of what you do in Bluebox:
...
```
Running this interactively in a terminal auto-starts `bluebox setup`. The installer supports **macOS and Linux natively; Windows requires WSL2**. Binary installs to `~/.bluebox/bin/bluebox`.
...
| `bluebox setup` | Run/re-run onboarding: auth, coding-agent skill install, opens Setup page for repo + ingest token |
| `bluebox auth login` / `logout` | Device-authorization-flow login; token stored in OS keyring (macOS Keychain, Linux Secret Service) or file fallback. Sign-in tokens expire after **14 days of inactivity**. |
| `bluebox skills list` / `skills get <name>` / `skills refresh` | Manage coding-agent skill files for agents not auto-supported by `bluebox setup` |
| `bluebox ask "<question>"` | Query live production data from the terminal; streams answer to stdout, progress to stderr |
| `bluebox otlp-endpoint` | Prints the workspace's OTLP ingest endpoint (URL only, never the token) |
| `bluebox workspace list` / `workspace switch <id>` | List/switch workspaces when a person belongs to more than one |
 
**Useful `bluebox ask` flags:** `--service`, `--env`, `--repo-url` (overrides git-origin auto-detection), `--entity-id` (pin an exact monitored entity), `--conversation-id` / `--continue` (resume a thread), `--workspace` (target a workspace for one call without changing the saved default).
 
**Environment variables:** `BLUEBOX_TOKEN` (single-invocation token for CI/scripting, never stored), `BLUEBOX_API_URL` (override API URL for one invocation), `BLUEBOX_DISABLE_KEYRING` (force file-based credential storage, useful on headless servers).
 
---
 
## Connections (Integrations)
 
Every connection has an **owner** (whoever created it). Connections are private-to-owner by default; other workspace members can view state but cannot reconnect, reauthorize, reconfigure, or delete unless the owner grants edit access or they are a workspace owner.
...
- IAM permission groups granted: Discover (`ListAgentSpaces`), Task management (`CreateBacklogTask`, `UpdateBacklogTask`, `GetBacklogTask`, `ListExecutions`), Investigation (`ListJournalRecords`, `SendMessage`, `CreateChat`).
...
### Kiro
- **Kiro IDE:** `bluebox setup` auto-detects and installs skills. Skills load on demand (not at session start); add a steering file (`bluebox skills get bluebox-overview > .kiro/steering/bluebox-overview.md`, committed to the repo) so every session has Bluebox context automatically.
...
```
Open a new terminal, or add this to the shell startup file manually if the installer's automatic `PATH` edit didn't take.
...
- **Coding agent support:** Claude Code and Kiro fully supported; GitHub Copilot, Cursor, OpenCode experimental; any other agent needs manual skill installation.
- **OS support (CLI):** macOS and Linux natively; Windows via WSL2 only.
...
| **Connection** | A link between Bluebox and an external system (e.g., AWS DevOps Agent, Kiro). Has an owner; other workspace members can view but not reconfigure it unless granted edit access. |
...
- Custom/unsupported coding agent skill installation issues beyond the documented manual `bluebox skills get` flow → likely needs Bluebox support (support@bluebox.ai).
